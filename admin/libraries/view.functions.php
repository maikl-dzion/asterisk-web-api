...
function redirect_standard( /* Note. Read the next line. Variable No of Params */ ) {
    ...
    $url = returnApiPostResult($args, $url);
    $url = returnApiGetResult($args, $url);
	redirect($url);
}
...
function redirect_standard_continue( /* Note. Read the next line. Varaible No of Params */ ) {
    ...
    if(!empty($_POST['request_api_alliance'])) {
        if($_POST['display'] == 'extensions')  {
            $url .= '&request_api_alliance=1';
        }
        else {
            allianceApiReturnPostResponse(array('args' => $args, 'url' => $url));
        }
    }
