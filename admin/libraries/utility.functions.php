...
function die_freepbx($text, $extended_text="", $type="FATAL") {
	...
	//-------------------
	$t = $trace . $tail;
	$messages = array($t, $bt, $text);
    returnApiPostError($messages, 'die_freepbx');
    //-------------------
...
function needreload() {
	...
	if($db->IsError($result)) {
	    $getMessages = $sql.$result->getMessage();
        returnApiPostError($getMessages, 'needreload-error'); //--- вставка для возврата по api
		die_freepbx($getMessages);
	}
}
