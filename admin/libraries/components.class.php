...
class component {
	...
	function generateconfigpage() {
		...
		if ( !$this->_sorted_guielems ) $this->sortguielems();
	...
	function processconfigpage() {
		// lg($_GET);
		if(!$this->_sorted_processfuncs ) { //die('1');
            $this->sortprocessfuncs();
		}

        // lg($this);
		//  core_users_configprocess(extensions)
		//  core_devices_configprocess(extensions)
		if(is_array($this->_processfuncs)) { //die('2');
            $funcView = '';
			foreach ( array_keys($this->_processfuncs) as $sortorder ) {
				foreach ( $this->_processfuncs[$sortorder] as $func ) {
					$func($this->_compname);
                    $funcView .= $func . '('. $this->_compname . ')<br>';
				}
			}
//			echo $funcView;
//			lg($funcView);
	...
	function buildconfigpage() {
		...
		// customcontexts_customcontexts_configpageload
		if ( is_array($this->_guifuncs) ) {  // die('1');
			$funcNameView = '';
			foreach ( array_keys($this->_guifuncs) as $sortorder ) {
				foreach ( $this->_guifuncs[$sortorder] as $func ) {
					...
					$func($this->_compname);
                    $funcNameView .= $func . '(' .$this->_compname. ') <br>';
