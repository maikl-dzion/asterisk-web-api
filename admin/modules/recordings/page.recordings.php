...
function recording_addpage($usersnum) {
    ...
    if (isset($_FILES['ivrfile']['tmp_name']) &&
        ...
        $successFileName =  "<h6>"._("Successfully uploaded")." ".$_FILES['ivrfile']['name']."</h6>";
        $rname = rtrim(basename($_FILES['ivrfile']['name'], $suffix), '.');

        $respArgs = array('suffix' => $suffix, 'rname' => $rname, 'path' => $destfilename);
        returnApiPostResult($respArgs);
    }
  ...
  <p></p>
	<form enctype="multipart/form-data" name="upload" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="POST">
		...
		<input type="button" value="<?php echo _("Upload")?>"
               onclick="document.upload.submit(upload); alert('<?php echo addslashes(_("Please wait until the page reloads."))?>');" tabindex="<?php echo ++$tabindex;?>"/>
	</form>
...
function recording_sidebar($id, $num) {
        ...
        ?>
        <div class="rnav"><ul>
        <li><a id="<?php echo empty($id)?'current':'nul' ?>
             " href="config.php?display=recordings&amp;usersnum=<?php echo urlencode($num) ?>"><?php echo _("Add Recording")?>
            </a></li>
