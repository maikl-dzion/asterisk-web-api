...
function ringgroups_add($grpnum,$strategy,$grptime,$grplist,$postdest,$desc,
						$grppre='',$annmsg_id='0',$alertinfo,$needsconf,$remotealert_id,
						$toolate_id,$ringing,$cwignore,$cfignore,$changecid='default',
						$fixedcid='',$cpickup='', $recording='dontcare') {
	global $db;
	global $astman;
	...
	if(is_array($extens)) {
		foreach($extens as $exten) {
			if ($exten[0]===$grpnum) {
                allianceApiGetResponse($_POST, array('saveStatus' => false), 'add');
	...
	$sql = "INSERT INTO ringgroups (grpnum, strategy, grptime, grppre, grplist, annmsg_id, postdest, description, alertinfo, needsconf, remotealert_id, toolate_id, ringing, cwignore, cfignore, cpickup, recording) 
                            VALUES ('".$db->escapeSimple($grpnum)."', '".$db->escapeSimple($strategy)."', ".$db->escapeSimple($grptime).", '".$db->escapeSimple($grppre)."', '".$db->escapeSimple($grplist)."', '".$annmsg_id."', '".$db->escapeSimple($postdest)."', '".$db->escapeSimple($desc)."', '".$db->escapeSimple($alertinfo)."', '$needsconf', '$remotealert_id', '$toolate_id', '$ringing', '$cwignore', '$cfignore', '$cpickup', '$recording')";
	$results = sql($sql);
  ...
  // added. So consider this a start.
	if ($astman) {
		...
		$errMessage = "Cannot connect to Asterisk Manager with " . $amp_conf["AMPMGRUSER"] . "/".$amp_conf["AMPMGRPASS"];
        allianceApiGetResponse($_POST, array('saveStatus' => false, 'errMessage' => $errMessage), 'add');
		die_freepbx($errMessage);
	}
