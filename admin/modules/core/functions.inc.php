...
function core_did_del($extension, $cidnum)
{
    ...
    $result = sql($sql);
    allianceApiGetResponse($_GET, array( 'saveStatus' => $result, 'data' => $extension), 'did-del');
}
...
function core_did_add($incoming, $target = false)
{
    ...
    if (empty($existing)) {
        ...
        allianceApiGetResponse($_POST, array( 'saveStatus' => false, 'errMessage' => 'Такой маршрут уже существует - '.$existing), 'add-did');
        echo "<script>javascript:alert('" . _("A route for this DID/CID already exists!") . " => " . $existing['extension'] . "/" . $existing['cidnum'] . "')</script>";
        return false;
    }
}
...
function core_devices_add($id, $tech, $dial, $devicetype, $user, $description, $emergency_cid = null, $editmode = false)
{
    ...
    // позаботьтесь о sip / iax / zap config
...
function core_users_add($vars, $editmode = false) {
    ...
    $resultInsert = sql($sql);  // СОХРАНЕНИЕ ВНУТРЕННЕГО НОМЕРА

    //-- ЗДЕСЬ СТАВИМ ЗАЦЕПКУ ------
    // allianceApiHandler($vars, $resultInsert, 'add');
    // lg($resultInsert);
...
function core_users_get($extension)
{
    ...
    if(!empty($_GET['action'])) {
        if($_GET['action'] == 'getUserRecordInfo') {
...
function core_users_del($extension, $editmode = false)
{
    ...
    if (DB::IsError($results)) {
        // allianceApiHandler($vars, $res, $action)
        die_freepbx($results->getMessage() . $sql);
    }
...
function core_trunks_add($tech, $channelid, $dialoutprefix, $maxchans, $outcid, $peerdetails,
                         $usercontext, $userconfig, $register, $keepcid, $failtrunk, $disabletrunk,
                         $name = "", $provider = "", $continue = "off", $dialopts = false)
{
    global $db;
...
function core_trunks_backendAdd($trunknum, $tech, $channelid, $dialoutprefix, $maxchans, $outcid, $peerdetails, $usercontext, $userconfig, $register, $keepcid, $failtrunk, $disabletrunk, $name, $provider, $continue, $dialopts = false)
{
    ...
    $sql = "
		...
		(`trunkid`, `name`, `tech`, `outcid`, `keepcid`, `maxchans`, `failscript`, 
		 `dialoutprefix`, `channelid`, `usercontext`, `provider`, `disabled`, `continue`)
		VALUES (
			'$trunknum',
...
function core_users_configprocess() {
    ...
    if (!checkAstMan()) return false;

    //check if the extension is within range for this user
    if (isset($extension) && !checkRange($extension)) { // die('check');
        ...
        switch ($action) {
            ...
            case "edit":
                ...
                if (core_users_edit($extdisplay, $_REQUEST)) {
                    //-- ЗДЕСЬ СТАВИМ ЗАЦЕПКУ ------
                    // allianceApiHandler($_REQUEST, 1, 'edit');
                    needreload();
                    redirect_standard_continue('extdisplay');
                } else {
                    // allianceApiHandler($_REQUEST, 0, 'edit');
...
function core_devices_configprocess()
{
    ...
    // убедитесь, что мы можем подключиться к Asterisk Manager
    if (!checkAstMan()) return false;

    //создавать vars из запроса
    $tech = $action = null;
    extract($_REQUEST);
    ...
    // фиксированные пользователи только в режиме extensions
    if ($display == 'extensions') {
        $devicetype = 'fixed';
