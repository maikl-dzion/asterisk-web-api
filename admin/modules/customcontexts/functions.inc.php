...
function customcontexts_hookGet_config($engine)
{
    ...
    switch ($engine) {
        case 'asterisk':
            $sql = 'UPDATE customcontexts_includes_list SET missing = 1 
                    WHERE context NOT IN (SELECT context FROM customcontexts_contexts_list WHERE locked = 1)';
            $db->query($sql);
...
function customcontexts_extensions_configpageload()
{
    ...
    $currentcomponent->addguielem('Device Options', 
                                   new gui_selectbox('customcontext', $currentcomponent->getoptlist('contextssel'), 
                                   $curcontext, 'Custom Context',
                                   'You have the ' . customcontexts_getmodulevalue('moduledisplayname') . ' Module installed! You can select a custom context from this list to limit this user to portions of the dialplan you defined in the ' . customcontexts_getmodulevalue('moduledisplayname') . ' module.', true, "javascript:if (document.frm_extensions.customcontext.value) {document.frm_extensions.devinfo_context.value = document.frm_extensions.customcontext.value} else {document.frm_extensions.devinfo_context.value = 'from-internal'}"));
...
function customcontexts_customcontexts_configprocess()
{
    ...
    // customcontexts_customcontexts_configprocess
    // lg($action);
    // addslashes
    switch ($action) {
        case 'add':
    ...
    returnApiPostResult(array($action, 'customcontexts_customcontexts_configprocess'));

    allianceApiReturnPostResponse(array('saveStatus' => true, 'context' => $context, 'action' => $action));
...
{
    global $db;
    $sql = "INSERT customcontexts_contexts 
                   (context, description, dialrules, faildestination, featurefaildestination, failpin, featurefailpin) 
            VALUES ('$context','$description','$dialrules','$faildest','$featurefaildest','$failpin','$featurefailpin')";
...
//    allianceApiGetResponse($_REQUEST, $responseData, 'add-context');
    ...
    needreload();
    return $saveResult;
    // returnApiPostResult($saveResult);
}
...
function customcontexts_customcontexts_edit($context, $newcontext, $description, $dialrules, $faildest, $featurefaildest, $failpin, $featurefailpin)
{
    ...
    $sql = "UPDATE customcontexts_contexts SET 
                context = '$newcontext', 
                description = '$description', 
                dialrules = '$dialrules', 
                faildestination = '$faildest', 
                featurefaildestination = '$featurefaildest', 
                failpin = '$failpin', 
                featurefailpin = '$featurefailpin' 
            WHERE  context = '$context'";
    // lg($sql);
    $db->query($sql);
    needreload();
...
function customcontexts_customcontexts_editincludes($context, $includes, $newcontext)
{
    ...
    foreach ($includes as $key => $val) {
        if ($val[allow] <> 'no') {
            ...
            $sql = "INSERT customcontexts_includes 
                      (context, include, timegroupid, sort , userules) VALUES 
                      ('$newcontext','$key' , $timegroup , $sort, '$userules')";
            $db->query($sql);
        }
    }
